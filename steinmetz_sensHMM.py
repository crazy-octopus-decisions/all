...
#this is binary HMM for repeat vs switch regime 
class HMM:
    def __init__(self,reCoef,swCoef,decay):
        # states=repeat/switch
        self.reCoef = reCoef
        self.swCoef = swCoef
        self.decay = decay
        #belief in state rep/swi
        self.beliefs = np.array([0.,0.])
        self.prevAct = 0
    
    def updateBeliefs(self,mouseAct): # action is observation
        if mouseAct == self.prevAct: # repeat
            self.beliefs[0] += self.reCoef
            #self.beliefs[1] *= self.decay
        else: # switch
            self.beliefs[1] += self.swCoef 
            #self.beliefs[0] *= self.decay
        self.prevAct = mouseAct
        self.beliefs *= self.decay
...
class sensoryFunc:
    ...
    def actionProbs(self,contrasts):
        ...
        if conDiff == 0 and (contrasts[0] ==0 and contrasts[1] == 0):
            conDiff = contrasts[0]+contrasts[1] #hacky way to make sure to go when both stimuli equal and != 0
...
class SensoryQ:
    def __init__(self,dat,goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta,reCoef,swCoef,decay,hmmDegree):
        self.sens = sensoryFunc(goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta)
        self.hmm = HMM(reCoef,swCoef,decay)
        self.hmmDegree = hmmDegree
        self.mouse_action = dat['response'] +1 # to make it 0,1,2 rather than -1,0,1
        self.mouse_reward = np.array(dat['feedback_type'])
    ...
    def run_model(self):
        for i in range(self.elen):
          curr_act = self.mouse_action[i]
          belief = self.hmm.beliefs
          belief = scipy.special.softmax(belief)
          cons = np.array([self.leftCons[i],self.rightCons[i]])
          sens_probs = self.sens.actionProbs(cons)
          if i > 0:
              prev_act = int(self.mouse_action[i-1])
              belActProbs = np.ones(3)
              belActProbs*= belief[1]/2 # multiple by switch
              belActProbs[prev_act] = belief[0] # prob in repeat regime
              confidence = abs(belief[0] - belief[1])
              sens_probs = ((1-confidence) * sens_probs) + (confidence * belActProbs)          
          sens_probs = [max(0,p) for p in sens_probs]
          precisionError = 1 - np.sum(sens_probs) # tiny errors break softmax
          sens_probs[np.argmax(sens_probs)] += precisionError
          self.mod_acts.append(np.random.choice([0,1,2],p=sens_probs))
          self.hmm.updateBeliefs(curr_act)
          self.mod_liks.append(sens_probs[int(self.mouse_action[i])])
          self.mod_probs.append(sens_probs)
          
def testParamsSensQ(params,dat):
    goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta,reCoef,swCoef,decay,hmmDegree = params
    #print(params)
    #errs = []
    for p in range(1):
        sens = SensoryQ(dat,goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta,reCoef,swCoef,decay,hmmDegree)
        sens.run_model()
        liks = sens.mod_liks
...
for i in alldat:
  ...
  dat = i
  # bounds = bounds for params during optimisation
  res = de(testParamsSensQ, bounds=[(1e-1,1e+1),(1e-1,1e+1),(0.8,1.5),(0.8,1.5),(-1e+1,1e+1),(-1e+1,1e+1),(-1.2,1.2),(-1.2,1.2),(-10.,10.),(-10.,10.),(0.,0.9),(0.,1.)],args=(dat,)) # , needed in args for tuple
  print(res)
  acclist = []
  for j in range(1):
    sensQ = SensoryQ(dat,res.x[0],res.x[1],res.x[2],res.x[3],res.x[4],res.x[5],res.x[6],res.x[7],res.x[8],res.x[9],res.x[10],res.x[11])
    sensQ.run_model()
    #hacky way of getting accuracy for param settings - will lead to variance in accuracy, but the mean should be correct and unbiased
  ...
  np.save("paramsSensHMM.npy",paramsSensQ)
...
def genPlots(sessionIndices):
    for testMouseNum in sessionIndices:
        ...
        for p in range(len(alldat)):
            ...
            negSumLogLik = np.sum(np.log(tmeta.liks))
            nLL.append(negSumLogLik)
        #meanAcc = np.mean(correct,axis=0)
        #meanSame = np.mean(testAccs,axis=0)
        bics = []
        for i in nLL:
            bic = -2* i + len(paramsMeta[0]) * np.log(len(tmeta.mod_acts))
            bics.append(bic)
        se = np.std(bics,axis=0)/np.sqrt(100)
        
        meanBics = np.mean(bics,axis=0)
        
        
        testPs = paramsMeta[testMouseNum]
        testAccs = []
        correct = []
        meanSame = []
        nLL=[]
        ...
        #get std err
        #BIC
        #bic -2 * LL + numParam * log(nTrials)
        #avgNSLL = np.mean(nLL,axis=0) # mean bics instead
        """
        testPs = paramsSensQ[testMouseNum] 
        sensQ = SensoryQ(dat,testPs[0],testPs[1],testPs[2],testPs[3],testPs[4],testPs[5],testPs[6],testPs[7],testPs[8],testPs[9],testPs[10],testPs[11])
        sensQ.run_model()
        #hacky way of getting accuracy for param settings - will lead to variance in accuracy, but the mean should be correct and unbiased
        acc = np.mean(sensQ.mod_liks)
        acts = sensQ.mod_acts
...
allModBics = np.load('allModBics.npy').tolist()

plt.bar(x = np.arange(len(bics)),height=bics,alpha=0.5,color = 'magenta')
plt.bar(x = np.arange(len(allModBics)),height=allModBics,alpha=0.5,color = 'cyan')
plt.xlabel('session')
plt.ylabel('BICs')

bicDiffs = [i-j for i,j in zip(allModBics,bics)]
plt.bar(x = np.arange(len(bicDiffs)),height=bicDiffs,alpha=0.5)
plt.xlabel('session')
plt.ylabel('BICs')
