...
it = 0
for i in nLLMeta:
    bics.append(2* i + len(paramsMeta[0]) * np.log(len(alldat[it]['response'])))
    it+=1
...
class SensoryQ:
    def __init__(self,dat,goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta):
...
def testParamsSensQ(params,dat):
    goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta = params
    #print(params)
    #errs = []
    for p in range(1):
        sens = SensoryQ(dat,goNoGo,leftRight,gngTheta,lrTheta,noGoBias,rightBias,ngbTheta,rbTheta)
        sens.run_model()
        liks = sens.mod_liks
...
for i in alldat:
  ...
  dat = i
  # bounds = bounds for params during optimisation
  res = de(testParamsSensQ, bounds=[(1e-1,1e+1),(1e-1,1e+1),(0.8,1.5),(0.8,1.5),(-1e+1,1e+1),(-1e+1,1e+1),(-1.2,1.2),(-1.2,1.2)],args=(dat,)) # , needed in args for tuple
  print(res)
  acclist = []
  for j in range(1):
    sensQ = SensoryQ(dat,res.x[0],res.x[1],res.x[2],res.x[3],res.x[4],res.x[5],res.x[6],res.x[7])
    sensQ.run_model()
    #hacky way of getting accuracy for param settings - will lead to variance in accuracy, but the mean should be correct and unbiased
  ...
  np.save("paramsSensQ.npy",paramsSensQ)
...
def genPlots(sessionIndices):
    for testMouseNum in sessionIndices:
        ...
        sensQ = SensoryQ(dat,testPs[0],testPs[1],testPs[2],testPs[3],testPs[4],testPs[5],testPs[6],testPs[7])
        sensQ.run_model()
        #hacky way of getting accuracy for param settings - will lead to variance in accuracy, but the mean should be correct and unbiased
        acc = np.mean(sensQ.mod_liks)
        acts = sensQ.mod_acts
...
randBICS=[]
for i in alldat:
    length = len(i['response'])
    BIC = -2*(sum([np.log(0.33)]*length))
    randBICS.append(BIC)

ranDiffs = [i-j for i,j in zip(randBICS,bics)]   
plt.bar(x = np.arange(len(ranDiffs)),height=ranDiffs,alpha=0.5,color = 'cyan')

plt.bar(x = np.arange(len(randBICS)),height=randBICS,alpha=0.5,color = 'cyan')
...
plt.xlabel('session')
plt.ylabel('BICs')
#next TODO: add history effects that aren't dpendent on reward : HMM?
